...
<div class="panel"><div class="fields"><NumberField label="N" name="Number of dice" value={count} min={1} max={100} onchange={(v)=>count=v}/><NumberField label="D" name="Sides per die" value={sides} min={2} max={1000} onchange={(v)=>sides=v}/></div><SeedControl bind:value={s}/><p class="mono note">Every die uses the same unbiased inclusive integer mapper. This is a reproducible simulation, not secure randomness.</p></div>
