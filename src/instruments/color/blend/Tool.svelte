<script lang="ts">
  ...
  import { BLEND_MODES, blendCss, blendOf, hex, lchOf, type BlendMode, type Lch } from '@duskresearch/primitives/design/color';
  ...
  const css = $derived(blendCss(a, b, mode, amount / 100));
...
<div class="surface bleed">
  <div class="layers">
    <button type="button" class="layer" data-copy={aHex} data-copy-label="hex">
      <span class="visually-hidden">Copy top color </span>
      <span class="chip" style:background-color={aHex}></span>
      <span class="label mono"><span class="muted" aria-hidden="true">A</span><span>{aHex}</span></span>
    </button>
    <div class="layer">
      <div class="chip result" style:background-color={resultHex} style:color={textOn(result)}>
        <Copy value={resultHex} label="result" primary class="big">{resultHex}</Copy>
      </div>
      <span class="label mono"><span class="muted">{names[mode]}</span><span>{pct(amount)}</span></span>
    </div>
    <button type="button" class="layer" data-copy={bHex} data-copy-label="hex">
      <span class="visually-hidden">Copy bottom color </span>
      <span class="chip" style:background-color={bHex}></span>
      <span class="label mono"><span class="muted" aria-hidden="true">B</span><span>{bHex}</span></span>
    </button>
  </div>
  {#if strips}
...
<div class="panel">
  <ColorControls name="Top" key="A" bind:color={a} />
  <ColorControls name="Bottom" key="B" bind:color={b} />
