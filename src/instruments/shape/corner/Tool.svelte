...
<div class="surface">
  <div class="comparison"><div><svg viewBox="0 0 100 100" role="img" aria-label="Superellipse-based corner" fill={shared.foreground}><path d={answer.path}/></svg><span>Superellipse K={1+s.smoothing}</span></div><div><div class="rounded" style:border-radius={`${s.cornerRadius}%`} style:background-color={shared.foreground}></div><span>Standard round</span></div></div>
...
<div class="panel">
  ...
  <div class="block"><p>Colored standalone SVG</p><Copy value={answer.svg} label="corner SVG" class="code">{answer.svg}</Copy></div>
