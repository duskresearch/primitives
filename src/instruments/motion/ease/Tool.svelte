...
<div class="surface">
  ...
  <div class="block"><p>{s.kind==='bezier'?'CSS cubic Bézier':'CSS sampled spring'}</p><Copy value={timing} label="CSS timing" primary class="code">{timing}</Copy></div>
</div>
<div class="panel">
  ...
  {#if s.kind==='spring'}<p class="mono note">Spring playback uses its measured settling time, <Copy value={`${ms}ms`} label="settling time">{ms}ms</Copy>. The shared Bézier duration remains saved for Export.</p>{/if}
</div>
<style>
