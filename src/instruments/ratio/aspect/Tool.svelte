<script lang="ts">
  ...
  const frame=$derived(result.answer ? fitPreviewFrame(result.answer.target.width,result.answer.target.height,280,230) : null);
  let loaded=false;$effect(()=>{const q=serializeWith(s,{targetWidth,targetHeight,keep});if(loaded)setQuery(q);loaded=true;});
</script>
<div class="surface">
  {#if result.answer}
    <div class="frame" style={`width:min(100%,${frame!.width}px);aspect-ratio:${result.answer.target.width}/${result.answer.target.height}`}><span>Target frame<br>{result.answer.solved.width} × {result.answer.solved.height}px</span></div>
...
<div class="panel">
  ...
  <p class="mono key">Desired ratio</p><div class="dimensions"><NumberField label="W" name="Target ratio width" value={targetWidth} min={1} max={1000} integer onchange={(v)=>targetWidth=v}/><NumberField label="H" name="Target ratio height" value={targetHeight} min={1} max={1000} integer onchange={(v)=>targetHeight=v}/></div>
...
<style>
  ...
  .frame{position:relative;min-width:0;min-height:0;flex-shrink:0;background:var(--paper);box-shadow:inset 0 0 0 1px var(--ink);overflow:hidden}
  .frame span{position:absolute;inset:0;display:grid;place-content:center;overflow:hidden;text-align:center}
