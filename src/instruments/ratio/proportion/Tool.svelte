<script lang="ts">
  ...
  const frame=$derived(result.answer ? fitPreviewFrame(result.answer.solved.width,result.answer.solved.height,280,230) : null);
  let loaded=false;$effect(()=>{const q=serializeWith(s,{name,keep});if(loaded)setQuery(q);loaded=true;});
</script>
<div class="surface">
  {#if result.answer}
    <div class="frame" style={`width:min(100%,${frame!.width}px);aspect-ratio:${result.answer.solved.width}/${result.answer.solved.height}`}><span>{result.answer.label}<br>{result.answer.solved.width} × {result.answer.solved.height}px</span></div>
