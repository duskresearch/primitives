...
export interface ShapeInput { form: ShapeForm; sides: number; radius: number; rotation: number }
...
export function form(x: FormInput) {
  ...
  const element = x.form === 'circle'
    ? `<circle cx="50" cy="50" r="${fixed(x.radius)}"${x.rotation ? ` transform="rotate(${fixed(x.rotation)} 50 50)"` : ''}/>`
    : `<path d="${path(vertices(x, x.inner))}"/>`;
...
export function polygon(x: PolygonInput) {
  ...
  return { css, points: x.points.map((p) => ({ x: p.x, y: p.y })), fillRule: 'nonzero' as const };
}
...
export function corner(x: CornerInput) {
  ...
  return { path: d, svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path d="${d}"/></svg>`, css, exponent, points: coords };
}
...
export function blob(x: BlobInput) {
  ...
  return { path: d, svg: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><path d="${d}"/></svg>`, knots };
}

export interface FaviconInput extends FormInput { foreground: string; background: string; padding: number }
...
export function favicon(x: FaviconInput) {
  ...
  if (!safeHex(x.foreground) || !safeHex(x.background)) throw new InputError('Colors must be six-digit hex values.');
  const element = form(x).element;
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" fill="${x.background}"/><g fill="${x.foreground}" transform="translate(${fixed(x.padding)} ${fixed(x.padding)}) scale(${fixed((100-2*x.padding)/100)})">${element}</g></svg>`;
